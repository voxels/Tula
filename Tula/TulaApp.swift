...
struct TulaApp: App {
    ...
    private var modelContent:[ModelViewContent] = TulaApp.defaultContent
    
    var body: some Scene {
        
        WindowGroup("Tula House", id: "ContentView", content: {

            ContentView( appState: appState, modelLoader: modelLoader, modelContent: modelContent, selectedModel: $selectedModel, placementModel: $placementModel )
        ...
        WindowGroup("Video Player", id:"VideoPlayer", content: {
                PlayerViewController(model: $playerModel)
                    ...
                    .task { @MainActor in
                        do {
                            try await playerModel.loadVideo(URL(string:selectedModel!.videoURLString)!,presentation: .inline )
                        } catch {
                            print(error)
                        }
        ...
        WindowGroup("Tula House", id: "VolumeSmallPlantView", content: {
            if let model = selectedModel {
                VolumeView(appState: appState, modelLoader: modelLoader, modelContent: modelContent, model: model, placementModel: $placementModel)
            } else {
                ProgressView("No model selected")
            }
        })
        .windowStyle(.volumetric)
        .defaultSize(width: 0.5, height: 1.0, depth: 0.5, in: .meters)
